"""
Test 1: IBL + trailing-edge correction + wedge.
...
3. Apply TE correction:  correction_dstar.
...
def run_single(airfoil: dict) -> dict:
    ...
    try:
        ...
        # ── TE slope angles ───────────────────────────────────────────────────
        target_slope = np.tan(np.deg2rad(ALPHA))
...
if __name__ == '__main__':
    ...
    print(f"  N_OUTER={N_OUTER}  forced transition at x=0")
    print(f"  TE correction:  blend_start={BLEND_START}")
