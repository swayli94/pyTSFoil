...
class SolverManager:
    ...
    def __init__(self, core):
        ...
        """
        self.core = core
        self.viscous_correction = ViscousCorrection(core)
        self.boundary_condition = BoundaryCondition(core)
        self.post_process = PostProcess(core)
    ...
    def solve(self) -> None:
        ...
        for iter_num in range(1, maxitm + 1):
            ...
            if outerr and flag_output == 1:
                cl_local = self.post_process.lift(clfact)
                cm_local = self.post_process.pitch(cmfact)
                ercirc = abs(dcirc)
